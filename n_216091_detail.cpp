...
lambda DECC(N s , int& maxError , double& aveError)
{
	lambda m = { 0 };
	int repeat = s.size() / m.size();
...
*/
lambda Dec(N sk, vector<N> c , int& maxError , double& aveError)
{
...
int main()
{
	...
	for(size_t times = 1 ; times < 200 ; times ++)
	{
		...
		cout << "明文 : " << m << endl;
		cout << "解密 : " << m_dec << endl;
		cout << "Equal(1)  or  not(0) : " << !((m ^ m_dec).count()) << endl; //原文xor解密後1的個數總和 = 0，取NOT = 1 
